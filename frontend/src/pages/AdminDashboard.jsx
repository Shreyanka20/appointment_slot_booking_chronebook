...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Navbar />
      <main className="max-w-7xl mx-auto px-6 md:px-10 py-10" data-testid="admin-root">
        <div className="mb-8">
          <p className="text-sm text-slate-500 mb-1">Admin</p>
          <h1 className="font-display text-3xl md:text-4xl font-bold text-slate-900">Control Room</h1>
        </div>

        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            {[
              { icon: <Users />, label: "Users", value: stats.users, bg: "bg-indigo-100 text-indigo-600" },
              { icon: <Calendar />, label: "Bookings", value: stats.bookings, bg: "bg-blue-100 text-blue-600" },
              { icon: <CheckCircle2 />, label: "Confirmed", value: stats.confirmed_bookings, bg: "bg-emerald-100 text-emerald-600" },
              { icon: <Layers />, label: "Meeting Types", value: stats.meeting_types, bg: "bg-violet-100 text-violet-600" },
            ].map((s, i) => (
              <div key={i} className="nb-card p-5" data-testid={`stat-${s.label.toLowerCase()}`}>
                <div className={`w-10 h-10 rounded-xl ${s.bg} mb-3 flex items-center justify-center`}>{s.icon}</div>
        ...
        <h2 className="font-display text-2xl font-extrabold mb-3">All Users</h2>
        <div className="nb-card overflow-x-auto mb-10">
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.user_id} className="border-t border-slate-100 hover:bg-slate-50" data-testid={`user-row-${u.user_id}`}>
                  ...
                  <td className="p-3">
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${u.role === "admin" ? "bg-violet-100 text-violet-700" : "bg-blue-100 text-blue-700"}`}>{u.role}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <h2 className="font-display text-2xl font-extrabold mb-3">All Bookings</h2>
        <div className="nb-card overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {bookings.map((b) => (
                <tr key={b.booking_id} className="border-t border-slate-100 hover:bg-slate-50" data-testid={`booking-row-${b.booking_id}`}>
                  <td className="p-3 text-slate-600">{new Date(b.start_iso).toLocaleString()}</td>
                  <td className="p-3 font-medium">{b.host_name}</td>
                  <td className="p-3">{b.invitee_name}<br/><span className="text-xs text-slate-500">{b.invitee_email}</span></td>
